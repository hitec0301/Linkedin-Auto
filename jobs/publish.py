...
from lnp import log, tokens as token_mod
...
def resolve_stuck_rows(sheets: Sheets, config, rows, api: LinkedIn, author: str) -> None:
...
def flag(sheets: Sheets, config, row: Row, marker: str, title: str, body: str) -> None:
    ...
    if not already_flagged:
        sheets.write(row, {"Error": marker})
    logger.warning("refused to publish", extra={"row_id": row.ID, "reason": marker})
    if not already_flagged:
        alert(config, title, body, severity="warn", job=JOB)
...
def main() -> int:
    ...
    with job_guard(JOB, config):
        ...
        # The kill switch, before anything else. It has to be reachable from a
        # phone in five seconds, so it is one cell in the Config tab.
        if sheets.is_paused():
            logger.warning("PAUSED is set; exiting without publishing")
            print("PAUSED is TRUE in the Config tab. Nothing was published.")
            return 0

        rows = sheets.pipeline_rows()

        expired = sheets.expire_stale(rows, config.staleness_hours)
        if expired:
            alert(
                config,
        ...
        tokens = token_mod.load_fresh(
            config,
            alerter=lambda title, body: alert(config, title, body, severity="warn", job=JOB),
        )
        api = LinkedIn(config, tokens)
        author = api.person_urn()
        token_mod.cache_person_urn(config, tokens, author)
        sheets.set_config_value("PERSON_URN", author)

        if stuck:
            resolve_stuck_rows(sheets, config, rows, api, author)
        ...
        for row in ready[:limit]:
            ...
            sheets.transition(row, Status.POSTING, {"Error": ""})
            try:
                ...
                sheets.transition(row, Status.FAILED, {"Error": str(exc)[:900]})
                alert(
                    config,
        ...
        if published and not dry_run:
            sheets.set_config_value("LAST_PUBLISH", iso())
            report_health(sheets, config)
