...
def main() -> int:
    ...
    with job_guard(JOB, config):
        ...
        history_urls, history_titles = sheets.recent_index(history_days)
        ...
        if not results:
            raise RuntimeError("no feeds were checked; is config/sources.yaml empty?")
        if not candidates:
            alert(
                config,
        ...
        archived = sheets.archive_old_rows(int(config.get("sheet.archive_after_days", 90)))
        sheets.set_config_value("LAST_CURATE", iso())
