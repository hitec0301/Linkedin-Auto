...
from lnp import drafting, log, voice
...
def main() -> int:
    ...
    parser.add_argument("--print", dest="print_only", action="store_true",
                        help="draft and print without writing to the Sheet")
    ...
    with job_guard(JOB, config):
        ...
        post_count = sheets.post_count()
        published = sheets.published_rows()
        ...
        for row in pending_draft:
            context = voice.build_voice_context(
                config, angle=row.angle, published=published, post_count=post_count
            )
